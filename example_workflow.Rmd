...
Next is an optional argument for a second raster that can be used to filter the provided habitat/probability of use layer for the species of interest. This will be set to NULL for this example. If a filter map is provided, a cut.off value for the filter map should be added to the Parameter list (named filter.cutoff). If no filter cutoff value is provided a default of 0.95 will be used if all values in the filter map are $\le$ 1.
