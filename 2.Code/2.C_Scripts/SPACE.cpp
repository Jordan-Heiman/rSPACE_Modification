...
//[[Rcpp::export]]
double rdist_earth_c(double long1, double lat1, double long2, double lat2)
{	
	double newdist;
...
//[[Rcpp::export]]
double rdist_km_c(double x1, double y1, double x2, double y2)
{	
	double newdist;
...
extern "C" {
  void make_grid_c(double x[], double y[], double *grid_size, int *pixels, int grid[])
{
	double disttest = sqrt(*grid_size);

	//Size of grid in km (at narrowest)
	double dist_x, dist_y;
	dist_x = rdist_earth_c(x[0], y[0], x[*pixels-1], y[0]); 
	dist_y = rdist_earth_c(x[0], y[0], x[0], y[*pixels-1]);
	...
	{	
		...
		// Test distance from origin for each pixel
 		while(ok)
		{	
			test_x = (rdist_earth_c(x[origin], y[test], x[test], y[test]) <= disttest);
			test_y = (rdist_earth_c(x[test], y[origin], x[test], y[test]) <= disttest);
			if(test_x && test_y)
			{
				grid[test] = cn;
				test++;
...
extern "C" {
  void sample_ind_c(double x[], double y[], int *N, double *buffer, int use[], int *maxid, int new_order[])
{
  int longlat;
...
//[[Rcpp::export(invisible = TRUE)]]
extern "C" {
void use_surface_c(double x_wolv[], double y_wolv[], int *N_wolv,
...
//[[Rcpp::export(invisible = TRUE)]]
extern "C" {
void calc_prob_c(double use[], int grid[], int detection[], double *detectionP, int *pixels, int *max_grid, double test[])
{
	int i;
